...
class scalarpow(object):
    ...
    Orad_i = initial value (at a = 0.001) of density parameter for radiation
    ...
    def integrand_bbks(self, k):
        return k**(self.ns + 2.)*Tbbks(k, self.Omega_m_z(0), self.Ob0, self.h)**2.*(self.Wf(k))**2.

    def integrand_wh(self, k):
        return k**(self.ns + 2.)*Twh(k, self.Omega_m_z(0), self.Ob0, self.h)**2.*(self.Wf(k))**2.
